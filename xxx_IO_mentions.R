...
cterms <- tibble(
  iso2c = c(# "EU", 
    "UN", "AU", "ASEAN", # "EUR", 
    "AFR", "AME", "OCEA", "ASI", "BRICS", "South"),
  cregex = c(# "\\beuropean union\\b|\\beu\\b|\\beuropean community\\b", 
    "\\bUNITED NATIONS\\b|\\bUnited Nations\\b|\\bUN\\b",
    "\\bAFRICAN UNION\\b|\\bAfrican Union\\b|\\bAU\\b",
    "\\bASEAN\\b",
    # "\\beurope(ean(s)?)?\\b",
    "\\bAfrica(n)?\\b|\\bafrica(n)?\\b|\\bAFRICA(N)?\\b",
    "\\bAmerica(n|s)?\\b|\\bamerica(n|s)?\\b|\\bAMERICA(N|S)?\\b",
    "\\bOceania(n)?\\b|\\boceania(n)?\\b|\\bOCEANIA(N)?\\b",
    "\\bAsia(n)?\\b|\\basia(n)?\\b|\\bASIA(N)?\\b",
    "\\bBRICS\\b",
    "\\bglobal south\\b|\\bGlobal South\\b"
  )
)
...
sent <- read_rds(paste0(data_path, "cleaned_data/data_sentlevel.rds")) %>% 
  ...
  filter(str_detect(text_sent, str_c(cterms$cregex, collapse = "|"))) %>% 
  # Year var
  mutate(year = str_extract(date, "^[0-9]{4}"))

IOmentions <- 
  sent %>% mutate(
    UN = str_count(text_sent, cterms$cregex[1]),
    AU = str_count(text_sent, cterms$cregex[2]),
    ASEAN = str_count(text_sent, cterms$cregex[3]),
    AFR = str_count(text_sent, cterms$cregex[4]),
    AME = str_count(text_sent, cterms$cregex[5]),
    OCEA = str_count(text_sent, cterms$cregex[6]),
    ASI = str_count(text_sent, cterms$cregex[7]),
    BRICS = str_count(text_sent, cterms$cregex[8]),
    South = str_count(text_sent, cterms$cregex[9])
  ) %>% select(sentence_id, year, UN:ncol(.)
               ) %>% 
  pivot_longer(cols = 3:ncol(.), names_to = "iso2c", values_to = "mentions") %>% # Easier to handle
  filter(mentions > 0 & iso2c == "South"
         )
...
#   select(c(sentence_id, year, EU = EU_expl, AU =
#            EU_expl:OCEANIA_total)) %>% # Only few meta and indvidual country indicators
#   pivot_longer(cols = 3:ncol(.), names_to = "iso2c", values_to = "mentions") %>% # Easier to handle
...
write_rds(cm2, paste0(data_path, "CountryMentions/IOs_phrase-words_vec.rds"))
