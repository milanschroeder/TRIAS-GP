...
#   select(c(sent_id, year, AD:ncol(.))) %>% # Only few meta and indvidual country indicators
...
cm_long <- read_rds(paste0(data_path, "CountryMentions/EN_CMs_sentlevel.rds")) %>%  # just read filtered version directly
  ...
  select(sent_id, year, AD:last_col()) %>% # Only few meta and indvidual country indicators
...
cterms <- 
  read_rds("data/country_dictionary.rds") %>%
  select(cregex = regex_capture_full, iso2c)



# Enrich country mentions with country regex and full text of respective sentence ####
cm2 <- read_rds(paste0(data_path, "CountryMentions/CMs_sentlevel_EN_long.rds")) %>% 
  left_join(cterms, by = "iso2c") %>% 
  left_join(read_rds(paste0(data_path, "cleaned_data/data_sentlevel.rds")) %>% select(sent_id, text_sent), by = "sent_id") %>% 
...
# save time:
cm2 %<>% distinct(text_sent, iso2c, .keep_all = T) 
...
cphrases <-  
  cm_long %>% 
  left_join(read_rds(paste0(data_path, "cleaned_data/data_sentlevel.rds")) %>% select(sent_id, text_sent), by = "sent_id") %>% 
  mutate(text_sent = str_replace_all(text_sent, "\\s+", " ")) %>% 
  left_join(., cm2, join_by(text_sent, iso2c)) # join repeated cases again (timesaver)



write_rds(cphrases, paste0(data_path, "CountryMentions/allCMs_sentlevel_phrase-words.rds"))
