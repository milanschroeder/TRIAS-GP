entity_phrase <- function(text = character(NULL), 
  ...
  if(plot == T) {
    ...
    # All docs (even if no phrase could be extracted)
    base <- an %>% 
      distinct(doc_id)
    
    # All extracted phrases
    phrase_data <- an %>% 
      filter(!is.na(phrase), phrase != "entity", pos != "PUNCT") %>%
      group_by(doc_id) %>% 
      summarise(phrase_words = paste0(token, collapse = ","), .groups = "drop")
    
    # Join: preserve all rows in `base`, fill missing phrase_words with NA
    result <- base %>%
      left_join(phrase_data, by = c("doc_id")) 
    # %>%
